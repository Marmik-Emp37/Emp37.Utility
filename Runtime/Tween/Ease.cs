...
namespace Emp37.Utility.Tween
{
      public static class Ease
      {
            ...
            // using Robert Penner's Easing notations -
            ...
            public static float InOutElastic(float t, float overshoot = 1F, float period = 0.45F)
            {
                  ...
                  float s = period / (2F * PI) * Asin(1F / overshoot);
                  t *= 2F;
                  if (t < 1F) return -0.5F * (overshoot * Pow(2F, 10F * (t - 1F)) * Sin((t - 1F - s) * (2F * PI) / period));
                  return overshoot * Pow(2F, -10F * (t - 1F)) * Sin((t - 1F - s) * (2F * PI) / period) * 0.5F + 1F;
            }
      }
}