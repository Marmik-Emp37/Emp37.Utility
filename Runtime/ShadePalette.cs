...
namespace Emp37.Utility
{
	...
	public static class ShadePalette
	{
		...
		static ShadePalette()
		{
			...
			palette = new Color[length];

			foreach ((Shade shade, uint hex) in source)
			{
				byte r = (byte) (hex >> 16), g = (byte) (hex >> 8), b = (byte) hex;
				Color color = new(r / 255F, g / 255F, b / 255F, 1F);

				palette[(int) shade] = color;
			}
		}

		/// <summary>
		/// Gets a random <see cref="Color"/> from the palette.
		/// </summary>
		public static Color Random => palette[UnityEngine.Random.Range(0, palette.Length)];

		#region Extensions
		public static Color ToColor(this Shade shade, float alpha = 1F)
		{
			Color color = palette[(int) shade];
