      ...
      public static class Extensions
      {
            public static FieldInfo GetField(this SerializedProperty property, BindingFlags flags = DEFAULT_FLAGS)
            {
                  ...
                  for (int last = path.Length - 1, i = 0; i <= last; i++)
                  {
                        ...
                        bool isArrayElement = segment.Contains('[');
                        string name = isArrayElement ? segment[..segment.IndexOf('[')] : segment;

                        field = FindField(name, currentType, flags);
                        if (i == last || field == null) break;

                        Type fieldType = field.FieldType;
                        currentType = isArrayElement ? fieldType.GetElementType() ?? fieldType.GetGenericArguments().FirstOrDefault() : fieldType;
                  }
